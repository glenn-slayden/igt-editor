...
namespace alib.Wpf.complex
{
	using String = System.String;


	//public static DataTemplate Load(String fn)
	//{
	//	fn = Path.GetFullPath(fn);
	//	if (!File.Exists(fn))
	//		throw new Exception();
	//	return (DataTemplate)XamlReader.Parse(File.ReadAllText(fn));
	//}
	...
	public sealed class ComplexGroupDataTemplateSelector : DataTemplateSelector
	{
		...
		public enum DiscoveryMethods
		{
			...
			/// <summary> use the item type to find the DataTemplate based on it's DataType property </summary>
			Type = 0x02,

			/// <summary> scan the item types interfaces to find the DataTemplate based on it's DataType property </summary>
			Interface = 0x04,

			/// <summary> scan the item types type hierachy to find the DataTemplate based on it's DataType property </summary>
			Hierarchy = 0x08,
